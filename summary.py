...
class partitionparser:
    ...
    def summary(self, fout = "", region = 1.0, bnmi = False, ML_par = None, ml_spe_setting = None, sp_setting = [], plot = True):
        ...
        if fout!="":
            ...
            bestpar = self.combine_simple_heuristic(tpartitions = tpartitions, pmap = pmap, idxpars = idxpars, fo = fout + "_PTPhSupportPartition", sp_setting = sp_setting, plot = plot)
            self.combine_simple_heuristic_spart(tpartitions = tpartitions, pmap = pmap, idxpars = idxpars, fo = fout + "_PTPhSupportPartition.spart", sp_setting = sp_setting, plot = plot)

            if bnmi:
                self.combine_max_NMI(tpartitions = tpartitions, pmap = pmap, fo = fout + "_PTPhNMIPartition.txt")
                self.combine_max_NMI_spart(tpartitions = tpartitions, pmap = pmap, fo = fout + "_PTPhNMIPartition.spart")

            if ML_par != None:
                self.combine_max_LLH(bestpar = ML_par, tpartitions = tpartitions, pmap = pmap, fo = fout + "_PTPMLPartition.txt", spe_setting = ml_spe_setting, plot = plot)
                self.combine_max_LLH_spart(bestpar = ML_par, tpartitions = tpartitions, pmap = pmap, fo = fout + "_PTPMLPartition.spart", spe_setting = ml_spe_setting, plot = plot)
